...
my @keys = (
    'EXP_NAME', 'EXP_ID', 'CHIP_ID', 'CAM_ID', 'FAKE_ID', 'WARP_ID', 'DIFF_ID', 
    'DIFF_POS', 'MJD-OBS', 'RA', 'DEC', 'TEL_ALT', 'TEL_AZ', 'EXPTIME', 
    'ROTANGLE', 'FILTER', 'AIRMASS', 'OBSCODE', 'SEEING', 'MAGZP', 'MAGZPERR', 
    'ASTRORMS', 'CMMTOBS', 'OBS_MODE', 'DIFFTYPE', 'SKY', 'SHUTOUTC',  
    'PLTSCALE', 'HIERARCH CMFVERSION'
#   , 'COMMENT'
);
...
#    'DIFF_POS' => [ Astro::FITS::CFITSIO::TBOOL(), 0, 'Diff sense of subtraction, T fwd F bkwd' ],
    ...
    'CMMTOBS' => [ Astro::FITS::CFITSIO::TSTRING(), 80, 'Comment associated to the exposure' ],
    'OBS_MODE' => [ Astro::FITS::CFITSIO::TSTRING(), 64, 'Observation mode (e.g. MD, 3PI...)' ],
    'DIFFTYPE' => [ Astro::FITS::CFITSIO::TSTRING(), 35, 'Difference type: WW (Warp-Warp Difference), WS (Warp-Stack Difference), SW (Stack-Warp Difference), WW (Warp-Warp Difference)' ],
    'SKY' => [ Astro::FITS::CFITSIO::TDOUBLE(), 0, 'Exposure average sky background' ],
    'SHUTOUTC' => [ Astro::FITS::CFITSIO::TSTRING(), 21, 'Camera exposure shutter open (UTC)' ],
    'PLTSCALE' => [ Astro::FITS::CFITSIO::TFLOAT(), 0, 'Plate scale' ],
    'HIERARCH CMFVERSION' => [ Astro::FITS::CFITSIO::TSTRING(), 7, 'PS1 DV1 or PS1 DV2 or PS1 DV3' ],
#    'COMMENT' => [ Astro::FITS::CFITSIO::TSTRING(), 80, 'Comment associated to the exposure' ],
);
...
my @rawattr_cols = qw(
    ...
    FLAGS2 N_FRAMES
    X_EXT Y_EXT X_EXT_SIG Y_EXT_SIG EXT_INST_MAG EXT_INST_MAG_SIG
    NPARAMS EXT_WIDTH_MAJ EXT_WIDTH_MIN EXT_THETA
    EXT_WIDTH_MAJ_ERR EXT_WIDTH_MIN_ERR EXT_THETA_ERR
    RA_EXT RA_EXT_SIGMA DEC_EXT DEC_EXT_SIGMA POSANG_EXT  
    EXT_FLUX EXT_CAL_MAG EXT_MAG_SIG EXT_CHISQ EXT_NDOF
);
my %rawattr_coldefs = (
    ...
    X_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    Y_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    X_EXT_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    Y_EXT_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_INST_MAG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_INST_MAG_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    NPARAMS => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],               # 50
    EXT_WIDTH_MAJ => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_WIDTH_MIN => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_THETA => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_WIDTH_MAJ_ERR => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_WIDTH_MIN_ERR => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_THETA_ERR => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    RA_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    RA_EXT_SIGMA => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    DEC_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    DEC_EXT_SIGMA => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],         # 60
    POSANG_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_FLUX => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_CAL_MAG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_MAG_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_CHISQ => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
    EXT_NDOF => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
);
...
#   $filename = $base_filename . "[0]";
    ...
    for my $key (@keys) {
        ...
        $value = $comment if ($key eq "COMMENT");
        _check_status($status, "error reading keyword $key");

        if (!defined($value)) {
            warn "Undefined keyword $key while reading $filename";
        }
        $value =~ s/^'//;
        $value =~ s/ *'$//;
    ...
    $self->{_extname} = $value if $value;

    # Read the version name.
    $self->{_fptr}->read_keyword("HIERARCH CMFVERSION", $value, $comment, $status);
    ...
    foreach $col (@cols) {
        ...
        if ($status == Astro::FITS::CFITSIO::COL_NOT_FOUND()) {
            warn "can't find required column $col in $filename";
            $colnum = -1; #Set column num to -1 to indicate that it was not found.
        }
    ...
    if ($self->{_ncols} >= 65) {
        ...
        $self->{_have_rawattr} = 1;
    }
    else {
        $self->{_have_rawattr} = 0;
    }
}
...
# 8   'FLAGS' => [ Astro::FITS::CFITSIO::TULONG(), '1J' ],
# 9   'DIFF_SKYFILE_ID' => [ Astro::FITS::CFITSIO::TLONG(), '1K1' ],
# 10  'RAWATTR_V3' => [
...
#   44  X_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   45  Y_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   46  X_EXT_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   47  Y_EXT_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   48  EXT_INST_MAG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   49  EXT_INST_MAG_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   50  NPARAMS => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],               
#   51  EXT_WIDTH_MAJ => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   52  EXT_WIDTH_MIN => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   53  EXT_THETA => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   54  EXT_WIDTH_MAJ_ERR => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   55  EXT_WIDTH_MIN_ERR => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   56  EXT_THETA_ERR => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   57  RA_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   58  RA_EXT_SIGMA => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   59  DEC_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   60  DEC_EXT_SIGMA => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],         
#   61  POSANG_EXT => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   62  EXT_FLUX => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   63  EXT_CAL_MAG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   64  EXT_MAG_SIG => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   65  EXT_CHISQ => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   66  EXT_NDOF => [ Astro::FITS::CFITSIO::TFLOAT(), '1E1' ],
#   ]
###################################################################
...
sub rejectrecord {
    my ($self, $row) = @_;
    my ($mag, $ra_sigma_deg, $dec_sigma_deg, $mag_sigma, $flags, $rawattr_v3) = @{$row}[5, 2, 4, 6, 8, 10];
    my $silly;
    my $dipole;
...
#exclude (DIFF_R_M < 20) && finite
    ...
    if ($rawattr_v3) {
        my ($psf_quality, $psf_qf_perfect, $diff_r_p, $diff_r_m) = @{$rawattr_v3}[7, 31, 38, 40];
