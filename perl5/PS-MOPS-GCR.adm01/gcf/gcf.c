...
/* d2math.c
 * 
 * Copyright (c) 2012 Sonia Keys Copyright (c) 2005,2006 Kyle Smalley,
 * Carl Hergenrother, Robert McNaught, David Asher
 * 
...
printf("trnas gcf->mRot... %f %f %f\n", gcf->mRot[0][0], gcf->mRot[0][1], gcf->mRot[0][2]);

        puts("*** rotation failed ***");
        //Modified by SC. 20140226
        //This:
        //exit(-1);
        //Was replaced by
        gcf->r0 = -1.;
        ...
        gcf->dr = -1.;
        return;
        //End of replacement/modifications
    }
...
double gcVel(gcfparam *gcf) {
   int nObs = gcf->nObs;
   double vel;

   // Vel is dist extent divided by time.
   int n1 = gcf->nObs - 1;
   double dra = gcf->rr * gcf->ntime[n1] - gcf->rr * gcf->ntime[0];
   double ddec = gcf->dr * gcf->ntime[n1] - gcf->dr * gcf->ntime[0];
   vel = sqrt(dra * dra + ddec * ddec) / gcf->ntime[n1];
   return vel * 180 / M_PI;
}
