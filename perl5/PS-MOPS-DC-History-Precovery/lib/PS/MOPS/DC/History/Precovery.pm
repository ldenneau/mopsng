package PS::MOPS::DC::History::Precovery;
...
sub new {
    ...
    my ($trackletId, $ephemerisDistance, $ephemerisUncertainty) =
        delete @args{qw(trackletId ephemerisDistance ephemerisUncertainty)};     # strip trackletId off input args

    my $self = PS::MOPS::DC::History::new($pkg, $inst, %args,
        eventType => $PS::MOPS::DC::History::EVENT_TYPE_PRECOVERY);
...
from history_precoveries hp
where event_id=?
SQL

            return PS::MOPS::DC::History::Precovery->new($inst,
...
where h.event_type='$PS::MOPS::DC::History::EVENT_TYPE_PRECOVERY'
...
PS::MOPS::DC::History::Precovery - Perl extension for history precovery events

=head1 SYNOPSIS

  use PS::MOPS::DC::History::Precovery;
  $evt = PS::MOPS::DC::History::Precovery->new($inst, %args);
  $evt->record();
