package PS::MOPS::DC::History::Link2;
...
sub new {
    ...
    my $self = PS::MOPS::DC::History::new($pkg, $inst, %args,
        eventType => $PS::MOPS::DC::History::EVENT_TYPE_LINK2);
...
select tracklet_id
from history_link2 d
where event_id=?
SQL

            return PS::MOPS::DC::History::Link2->new($inst,
...
where h.event_type='$PS::MOPS::DC::History::EVENT_TYPE_LINK2'
...
SQL
            ...
            push @hobjs, PS::MOPS::DC::History::Link2->new($inst,
...
PS::MOPS::DC::History::Link2 - Perl extension for history Link2 derivation events

=head1 SYNOPSIS

  use PS::MOPS::DC::History::Link2;
  $evt = PS::MOPS::DC::History::Link2->new($inst, %args);
  $evt->record();
