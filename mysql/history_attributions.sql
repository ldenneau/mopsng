...
drop table if exists history_attributions;
create table history_attributions(
    event_id bigint not null             comment 'Parent event ID (from history table)',
    tracklet_id bigint not null          comment 'Attributed tracklet ID',
    ephemeris_distance float not null    comment 'Predicted position minus actual, arcsecs',
