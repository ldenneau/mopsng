 ...
 * Note that there are still a few string_array functions in amdmex.h.
 * Some belong here, and some are esoteric that they should stay there. 
 *
...
/*needed to allow forward declaration of type string_array without full 
  #include am_string_array.h for the ASL C api*/
#ifndef STRING_ARRAY_TYPEDEF_DEFINED
#define STRING_ARRAY_TYPEDEF_DEFINED
...
string_array *mk_string_array_1(const char *x1);
...
/* Like mk_va_string_array(), above, but takes a va_list.  Last element
   of va_list must be NULL. */
...
int find_closest_index_in_sorted_string_array(string_array *sa, const char* string);
...
void line_count_unquoted(const char *string, int *r_num_unquoted_commas, int *r_num_unquoted_bars);
...
/* makes a string_array form row of string_matrix */
string_array *mk_string_array_from_string_matrix_row(string_matrix *sm,
						     int row);

/* refers to the string_array represented by row of string matrix
