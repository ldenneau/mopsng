/* File:        plates.c
   Author:      J. Kubica and L. Denneau
   Description: API for LinkTracklets
    ...
    for(j=0;j<track_array_size(t1);j++) {
      ...
      if(grp1 == grp2) {
        if((track_num_obs(S) > 1)&&(track_num_obs(F) >= 6)) {
          ...
          fprintf(fout,"%10f %10f %10f %10f %10f %10f ",track_RA(F),
                  track_DEC(F),track_vRA(F),track_vDEC(F),track_aRA(F),
                  track_aDEC(F));
...
void tracker_main(int argc,char *argv[]) {
  ...
  if(fname == NULL) {
    ...
    if(obs != NULL) {
      ...
      if(search_type < 5) {
        printf(">> Shifting the observation bounds. (");
        printf(curr_time());
        printf(")\n");
        simple_obs_array_compute_bounds(obs,NULL,&r_lo,&r_hi,&d_lo,&d_hi,
                                        &t_lo,&t_hi);
        ...
        simple_obs_array_compute_bounds(obs,NULL,&r_lo_n,&r_hi_n,&d_lo_n,
                                        &d_hi_n,&t_lo_n,&t_hi_n);
      ...
      printf(">> Flattening the Tracks to Plates ");
      printf(curr_time());
      printf("\n");
      ...
      printf(">> Unflattening the Tracks to Plates ");
      printf(curr_time());
      printf("\n");
      ...
      if(search_type < 5) {
        printf(">> Shifting the observation bounds, back (");
        printf(curr_time());
        printf(")\n");
      ...
      if(eval == TRUE) {
        ...
        roc = mk_track_array_roc_vec(obs,t2,filtered_true_groups,min_obs,
                                     min_sup,roc_thresh);
        printf("Full Track: There are %i true tracks with %i or more obs on %i or more days.\n",
               ivec_max(filtered_true_groups)+1,min_obs,min_sup);
        printf("Full Track: We found %i tracks with %i or more obs on %i or more days.\n",
        ...
        RJ_scoremat = mk_track_array_RJ_score_mat(t2,true_groups,FALSE);
        free_dym(RJ_scoremat);    
...
int linktracklets_main(int argc,char *argv[]) {
    int seed = int_from_args("seed",argc,argv,0);
...
static char tracker_main__doc__[] = "Main entry point for LinkTracklets.";

PyObject *wrap_linktracklets_main(PyObject *self, PyObject *args, PyObject *kw){
    ...
    char* file;
    char* trackfile = "tracks.obs";
    char* summaryfile = "tracks.sum";
    char* idsfile = "tracks.ids";
    char* scoresfile = "";
    char* fit_thresh = "0.0001";
    char* lin_thresh = "0.05";
    char* quad_thresh = "0.02";
    char* pred_thresh = "0.0005";
    char* vtree_thresh = "0.0002";
    char* min_overlap = "0.5";
    char* plate_width = "0.001";
    char* roc_thresh = "1.0";
    char* maxv = "1000.0";
    char* minv = "0.0";
    char* sigma = "0.0";
    ...
    char* endpts = "TRUE";
    char* eval = "FALSE";
    char* remove_subsets = "TRUE";
    char* remove_overlaps = "FALSE";
    char* allow_conflicts = "FALSE";
    char* fileout = "TRUE";
    ...
    /* TODO: do a better job at this: there is NO reason to parse args from 
             Python into strings and then have the C layer re-parse them! */
    /* Once parsed the keywords, fake a standard C argv */
    char *argv[] = {"linktracklets",
                    "file",
                    file,
                    "trackfile",
                    trackfile,
    ...
    linktracklets_main(31, argv);
    return(Py_BuildValue("i", 1));
};



/* Define the Python package and modules therein. */
static PyMethodDef linktracklets_methods[] = {
    {"linktracklets", 
...
static char linktracklets_module_documentation[] = 
"LinkTracklets Module Docs go here."
;

void initlinktracklets(void) {
    /* Create the module and add the functions */
	(void)Py_InitModule4("linktracklets", 
	                     linktracklets_methods,
                   	     linktracklets_module_documentation,
                         (PyObject *)NULL, 
                         PYTHON_API_VERSION);

	/* Check for errors */
	if(PyErr_Occurred())
        Py_FatalError("can't initialize module linktracklets");
}
